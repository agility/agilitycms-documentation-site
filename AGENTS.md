...
- **Key handling is deliberately narrow.** `/api/explorer/fetch-key` issues a key only for an instance the caller can already open in the CMS, and only the published-content `fetch` key, never `preview`. ⚠️ Upstream `GET mgmt.aglty.io/oauth/getfetchkey?guid=…` is **unauthenticated** — any guid, no token, live key (verified 2026-09-20) — so don't simplify that gate away. The key is held in component state only, never web storage: articles on this origin execute author-supplied `<script>`.
- **Management API is read-only in the explorer** (`allowWrites: false` in [lib/api-specs/registry.ts](lib/api-specs/registry.ts)). All 115 operations are documented; nothing wires "try it" to a write against a customer's live instance. Wiring it up needs the OAuth flow plus a confirmation step.

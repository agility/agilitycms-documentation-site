...
>
> ⚠️ **Adding a hand-written route under `app/`? Add it to `APP_PATHS` in [proxy.ts](proxy.ts)** — otherwise it 404s in production while working perfectly in `next dev`, because the check is skipped in dev and draft mode. Anything under `/api/*` is exempt. Current set: `/`, `/llms.txt`, `/robots.txt`, `/sitemap.xml`, `/404`, `/500`.

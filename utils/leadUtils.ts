...
const getLeadSourceDetailForForm = (formID: string | number): string => {
...
const getLeadSource = (): string | null | undefined => {
  if (localStorage && localStorage.getItem) {
    return localStorage.getItem(key);
  }
};

const getAutopilotSession = (): string | null => {
  var autopilotSessionElem = document.getElementById("_autopilot_session_id") as HTMLInputElement | null;
  var autopilotSession: string | null = null;
...
const getExternalReferrer = (): string | null => {
  if (document.referrer && document.referrer.length > 0) {
    ...
    const ignoreDomains = [
      "agilitycms.com",
      "localhost",
      "agilitycms-uat.azurewebsites.net",
    ];
...
const queryString = function (name: string, url?: string | null): string | null {
  /// <summary>
...
const getSetLeadSource = (): string | null | undefined => {

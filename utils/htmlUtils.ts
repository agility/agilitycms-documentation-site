...
const renderHTML = (html: string | null | undefined, mainSite?: boolean): { __html: string } => {
  if (!html) return { __html: "" };
  return { __html: cleanHTML(html, mainSite) };
};

const cleanHTML = (html: string, mainSite?: boolean): string => {
  if (!html) return "";
  ...
  for (const link of links) {
    ...
    link.setAttribute("style", null as any);

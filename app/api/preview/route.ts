...
export async function GET(request: NextRequest) {
	...
	const locale = searchParams.get("locale") || searchParams.get("lang") || process.env.AGILITY_LOCALES?.split(',')[0] || 'en-us'
	...
	if (nextConfig.basePath && !previewUrl.startsWith(nextConfig.basePath)) {
		previewUrl = `${nextConfig.basePath}${previewUrl === '/' ? '' : previewUrl}`
	}
	...
	if (ContentID) {
		...
		if (dynamicPath) {
			...
			if (nextConfig.basePath && !previewUrl.startsWith(nextConfig.basePath)) {
				previewUrl = `${nextConfig.basePath}${previewUrl}`
			}
		}
	}

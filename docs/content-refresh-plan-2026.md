...
### For the SDK teams — found 2026-07-30 by `check_sdk_drift.py` *(API/SDK issues, not doc bugs)*
- **`Agility.Management.SDK` has never shipped a stable release.** Twelve versions, all `-beta`, latest `1.0.11-beta`, so `dotnet add package Agility.Management.SDK` fails without `--prerelease`. Either ship a stable version or say "beta" on the tin.
- **Package id vs assembly name.** The package is `Agility.Management.SDK`; the assembly and namespace are `management.api.sdk`. That mismatch is what produced the broken install command in our own docs — worth aligning.
- **The .NET SDK is ~17 methods behind JavaScript** (64 vs 81 public methods). Missing groups entirely: `instanceMethods` (locales, Fetch API status), `serverUserMethods`, `webhookMethods`. Missing individually: `getContainerListPaged`, `batchWorkflowContent`, `batchWorkflowPages`, `getContentList`, `getContentHistory`, `getContentComments`, `getPageHistory`, `getPageComments`, `deleteFolder`, `renameFolder`, and five of the eight `batchMethods`. Every one has a REST endpoint, so these are wrapper gaps.
- **`ContentDefinitionTypeID` enum disagrees with the API.** The JavaScript SDK declares `List = 0, SingleItem = 1, Component = 2`; `GET /api/v1/types` → `contentModelTypes` gives `Item = 0, List = 1, Module = 2`, and a real Content List container reports `1`. The SDK enum looks off by one position and will mislabel containers.
- **`Locales` is typed as `class Locales extends Array<string>`**, and `getLocales` returns `Locales[]` — but the endpoint returns objects (`localeID`, `localeName`, `localeCode`, `enabled`, `custom`, `sortOrder`). The declared type doesn't describe the payload.
- **`getPageTemplateName` fetches a template by name and returns a `PageModel`.** The name says otherwise. `GetPageTemplateByName` (the .NET name) is the better one; renaming would be breaking, so at minimum document the alias.
- **`serverUserMethods.me(guid)` needs the GUID only to pick a region.** The endpoint (`GET /api/v1/users/me`) isn't instance-scoped, so `me("")` silently targets the default region — a foot-gun for non-US instances.
- Still open from the earlier merge: the `Webhook` schema leaks Azure Table Storage fields (`partitionKey`, `rowKey`, `eTag`, `timestamp`) into the public contract.
...
2. **[api-spec-drift](../.claude/skills/api-spec-drift/SKILL.md)** *(added 2026-07-30)* — accuracy: diffs the docs' API claims against the live OpenAPI specs for the **Management API** (`mgmt.aglty.io/swagger/v1/swagger.json`, 100 paths) and **Fetch API** (`api.aglty.io/swagger/v1/swagger.json`, 15 paths). Catches endpoints that don't exist, fields documented as required that aren't, parameter names absent from the spec, oversized page-size examples, and coverage gaps. Bundles `check_drift.py`, grades findings HIGH/MEDIUM/INFO, and exits non-zero on a HIGH so it can gate CI later.
   ...
   *Two scripts, both required:* `check_drift.py` (docs vs the REST specs) and
   `check_sdk_drift.py` *(added 2026-07-30)* (docs vs the published **SDK packages** —
   `@agility/management-sdk` on npm, `Agility.Management.SDK` on NuGet). The second
   exists because the first structurally cannot see SDK claims, and that is exactly
   where the serious errors were: a `dotnet add package` command naming the assembly
   instead of the package (so the .NET quickstart 404'd at step one), four methods
   documented as missing from JavaScript that JavaScript exports, a paged-listing
   section attributed to .NET when it only exists in JavaScript, and two parameters
   labelled ".NET only" that are in the JavaScript signature too. `--matrix` prints a
   verified JS ⇄ .NET method matrix; treat it as the only trustworthy source for
   cross-SDK availability tables. Needs `dnfile` for the .NET half — see the skill for
   why a `strings` scan can't prove absence.

   *The pattern worth remembering:* **errors clustered wherever nothing could
   contradict the prose.** Where ground truth existed the docs were broadly right.
   Free-form settings dictionaries are still unguarded — the only check there is
   reading a real model or container back from a live instance.
...
| 2026-07-30 | **SDK-surface audit — 5 of 9 Management SDK articles corrected (staged).** Checking the docs against the *packages* rather than the specs found errors the REST diff couldn't see. **1608** broken `dotnet add package` (assembly name, not package id; package is `Agility.Management.SDK`, prerelease-only) + completed and verified feature-gap table with a REST fallback per gap. **1614** paged-listing section was attributed to .NET but is JavaScript-only; four wrong "not available in JavaScript" notes; fabricated container `settings` object (real fields are top-level `defaultSortColumn`/`defaultSortDirection`/`numRowsInListing`); `displayName`/`description` don't exist on `Container`; hyphen-avoidance naming guidance added from our own GraphQL incident. **1611** `pageIDInOtherLocale`/`otherLocale` are not ".NET only"; documented the JavaScript argument-order trap and `linkExistingComponents`. **1615** entire model-field surface rebuilt — `ModelField` has no `referenceName`/`required`/top-level `defaultValue`, `settings` is a string→string dictionary, and field types are PascalCase (`Text`, `HTML`, `ImageAttachment`, `DropdownList`, `LinkedContentDropdown`…), not the invented lowercase set; points at `GET /model/field-types` as authoritative. **1612** expanded to close all four remaining coverage gaps (locale read + full REST locale management, `GET /api/v1/users/me` regional-host gotcha, `getFetchApiStatus` for publish→CDN propagation, `getfetchkey`/`getpreviewkey`, and the public `/api/v1/types` enum reference with verified `itemStates`/`workflowOperationTypes`/`contentModelTypes` values). Both checkers now report **0 HIGH**, validated by a negative control against the pre-fix wording. **All staged — not yet published.** |

...
- **Suggested article:** "Set Up the Agility MCP Server in Claude Code, Cursor & VS Code" — Developers (Extensibility), with a full tools table and example prompts. State plainly what the MCP cannot do (publish, delete, workflow) per current behavior.

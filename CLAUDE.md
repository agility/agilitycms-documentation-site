...
- **The Agility MCP can publish, unpublish, move workflow state, and delete** (`publish_content` / `unpublish_content`, `manage_content_workflow`, `publish_page` / `unpublish_page`, `delete_content_item`) — verified 2026-07-28; earlier docs said it couldn't. But `save_content_items` still always writes to **Staging** (`state` is ignored on save), so a saved edit is **not live** until you publish it. Publishing/deleting is outward-facing on the live docs site — confirm with a human first, and never imply a save went live until it's published.

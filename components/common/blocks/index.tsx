...
const BlockComponentDefinitions: Record<string, { component: any; useProse: boolean }> = {
  'header': {
    component: Heading,
    useProse: true
  },
...
const Blocks = ({ blocks, proseSize }: { blocks: any[]; proseSize?: string }) => {
  let blockRenderMarkup = null;
  //only work with Blocks we can handle...
  const blocksFiltered = blocks.filter((block: any, id: number) => {
  ...
  let blockGroupings: { useProse: boolean; blocks: { Component: any; block: any }[] }[] = [];

  blocksFiltered.map((block: any, idx: number) => {
